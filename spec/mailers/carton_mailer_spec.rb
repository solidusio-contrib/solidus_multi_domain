...
describe Spree::CartonMailer do
  ...
  describe 'from address' do
    ...
    context 'the order has a store' do
      ...
      context 'the store does have an email' do
        let(:store) { FactoryGirl.create(:store, email: 'store@example.com') }

        it { is_expected.to eq ['store@example.com'] }
      end

      context 'the store does not have an email' do
        let(:store) { FactoryGirl.create(:store, email: nil) }
