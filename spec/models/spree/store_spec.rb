...
describe Spree::Store do
  let!(:default_store) { create(:store, default: true, url: "default.com") }
  let!(:store_2) { create(:store, code: 'STORE2', url: 'freethewhales.com') }
  let!(:store_3) { create(:store, code: 'STORE3', url: "website1.com\nwww.subdomain.com") }

  it "should find stores by url" do
    by_url = Spree::Store.where('url like ?', '%www.subdomain.com%')
  ...
  it "should find the current store by url" do
    current_store = Spree::Store.find_by(code: 'website1.com')

    expect(current_store) == store_3
  end

  it "should find the current store by code" do
    current_store = Spree::Store.find_by(code: 'STORE2')
